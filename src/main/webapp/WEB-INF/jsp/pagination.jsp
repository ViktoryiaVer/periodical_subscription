...
<div id="pagination">
    <c:if test="${totalPages > 1}">
        <c:if test="${currentPage > 1}">
            <a href="${command}?page=1&page_size=${pageSize}">First</a>
        ...
        <c:if test="${currentPage < totalPages}">
            ...
            <a href="${command}?page=${totalPages}&page_size=${pageSize}">Last</a>
        </c:if>
    </c:if>
    <form id="page-size-form" action="${command}">
        ...
        <button class="btn btn-light" type="submit" title="Show on page">Show</button>
    </form>
</div>
